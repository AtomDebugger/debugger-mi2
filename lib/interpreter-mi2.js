...
export default class InterpreterMi2 {
  ...
  interpret(data: string) {
    ...
    for (let line of data.split('\n')) {
      ...
      if (/^\*running/.test(line)) {
        ...
        } else if (result.reason === 'end-stepping-range') {

          let message = this.adapter.adaptSessionMessage(result)
          this.emitter.emit('session', new SessionEvent('suspended', 'step', message))
  ...
  convertResult(value: string): Object {
    ...
    for (let i=0; i<value.length; i++) {
      if (value.charAt(i) === '"') {
        isQuoted = !isQuoted
      } else if (!isQuoted && value.charAt(i) == '{' || value.charAt(i) == '[') {
        scopeDepth++
      } else if (!isQuoted && value.charAt(i) == '}' || value.charAt(i) == ']') {
        scopeDepth--
      } else if (!isQuoted && value.charAt(i) == ',' && scopeDepth == 0) {
